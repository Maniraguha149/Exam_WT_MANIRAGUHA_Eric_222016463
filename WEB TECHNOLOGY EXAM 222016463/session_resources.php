...
<section>
  ...
  if ($_SERVER["REQUEST_METHOD"] == "POST") {
      ...
      $cvp = $connection->prepare("INSERT INTO session_resources(resource_id, session_id, resource_name, resource_type, file_path) VALUES (?, ?, ?, ?,?)");
      $cvp->bind_param("iisss", $resource_id, $session_id, $resource_name, $resource_type, $file_path);
  ...
  <h2>Table of session_resources</h2>
  <table border="1">
    <tr>
      <th>resource_id</th>
      <th>session_id</th>
    ...
    <?php
    include "database_connection.php";
    $sql = "SELECT * FROM session_resources";
    ...
    if ($result->num_rows > 0) {
        while ($row = $result->fetch_assoc()) {
            ...
            echo "<tr>
                    ...
                    <td><a style='padding:4px' href='delete_session_resources.php?resource_id=$resource_id'>Delete</a></td> 
                    <td><a style='padding:4px' href='update_session_resources.php?resource_id=$resource_id'>Update</a></td> 
                </tr>";
        }
    } else {
